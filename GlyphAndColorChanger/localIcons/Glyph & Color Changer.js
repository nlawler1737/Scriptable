...
async function presentIconPicker(co,ic,name) {
  ...
  let cssPath = fm.joinPath(fm.documentsDirectory(), "iconGlyphs/all.css")
  let ttfPath = fm.joinPath(fm.documentsDirectory(), "iconGlyphs/icons.ttf")
  let cssStr = fm.readString(cssPath)
  cssStr = cssStr.replace("--REPLACE_URL_HERE--", "data:font/ttf;base64,"+Data.fromFile(ttfPath).toBase64String())
  ...
  iconColors+="<div style='margin-top:50px;'>"
  for (i in colorKeys) {
  ...
  <body style="margin:0;text-align:center;min-height:200vh;">
    <div style="height:20vw;width:90vw;background:${colors[co]};color:white;padding:1vw;margin-left:4vw;border-radius:2vw" gotColor>
      <div>
        <i style="width:15vw;height:15vw;font-size:10vw;float:left;margin-top:1.5vw;" class="fa fa-${ic}" aria-hidden="true" gotIcon></i>
      </div>
